...
class ProductRequestRecordPage extends StatefulWidget {
...
class _ProductRequestRecordPageState extends State<ProductRequestRecordPage> {
  List<ProductRequestRecordModel> auth_record = [];
  List<GroupedProductRequestRecordModel> grouped_record = [];
  List<ProductRequestRecordModel> selected_record = [];
  List<ProductRequestRecordModel> pending_record = [];
  ...
  get_record() {
    List<ProductRequestRecordModel> all_record =
        Provider.of<AppData>(context).product_request_record;
    ...
    groups.forEach((key, value) {
      GroupedProductRequestRecordModel val =
          GroupedProductRequestRecordModel(date: key, record: value);
  ...
  // date picker tile
  Widget date_picker_tile(GroupedProductRequestRecordModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
  ...
  // record view
  Widget record_view(List<ProductRequestRecordModel> _record) {
    ...
    return Container(
      child: _record.isNotEmpty
          ? StickyGroupedListView(
              ...
              elements: _record,
              groupBy: (ProductRequestRecordModel element) =>
                  UniversalHelpers.get_date(element.recordDate!),
              groupComparator: (DateTime value1, DateTime value2) =>
                  value2.compareTo(value1),
              groupSeparatorBuilder: (ProductRequestRecordModel element) =>
                  group_separator(element.recordDate!),
              itemBuilder: (context, ProductRequestRecordModel element) =>
                  record_tile(element),
  ...
  // record tile
  Widget record_tile(ProductRequestRecordModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Text('Requested by : ', style: label_style),
                    SizedBox(width: 4),
                    Text(record.requestedBy?.nickName ?? 'No Staff',
                        style: main_style),
                    Expanded(child: Container()),
  ...
  Widget view_record_by_date(
    ...
    List<ProductRequestRecordModel> records = [];
    ...
    return FutureBuilder<List<ProductRequestRecordModel>>(
      initialData: records,
      future: records.isNotEmpty
          ? null
          : (date != null && isOffList)
              ? ProductStoreHelpers.get_selected_product_request_record(
...
class GroupedProductRequestRecordModel {
  List<ProductRequestRecordModel> record;
  DateTime date;

  GroupedProductRequestRecordModel({
