...
class BadProductRecordPage extends StatefulWidget {
  ...
  @override
  State<BadProductRecordPage> createState() => _BadProductRecordPageState();
}

class _BadProductRecordPageState extends State<BadProductRecordPage> {
  List<BadProductRecordModel> auth_record = [];
  List<GroupedBadProductRecordModel> grouped_record = [];
  List<BadProductRecordModel> selected_record = [];
  List<BadProductRecordModel> pending_record = [];
  ...
  get_record() {
    List<BadProductRecordModel> all_record =
        Provider.of<AppData>(context).bad_product_record;
    ...
    groups.forEach((key, value) {
      GroupedBadProductRecordModel val =
          GroupedBadProductRecordModel(date: key, record: value);
  ...
  // date picker tile
  Widget date_picker_tile(GroupedBadProductRecordModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
  ...
  // record view
  Widget record_view(List<BadProductRecordModel> _record) {
    ...
    return Container(
      child: _record.isNotEmpty
          ? StickyGroupedListView(
              ...
              elements: _record,
              groupBy: (BadProductRecordModel element) =>
                  UniversalHelpers.get_date(element.recordDate!),
              groupComparator: (DateTime value1, DateTime value2) =>
                  value2.compareTo(value1),
              groupSeparatorBuilder: (BadProductRecordModel element) =>
                  group_separator(element.recordDate!),
              itemBuilder: (context, BadProductRecordModel element) =>
                  record_tile(element),
  ...
  // record tile
  Widget record_tile(BadProductRecordModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Text('Staff Responsible : ', style: label_style),
                    SizedBox(width: 4),
  ...
  Widget view_record_by_date(
    ...
    List<BadProductRecordModel> records = [];
    ...
    return FutureBuilder<List<BadProductRecordModel>>(
      initialData: records,
      future: records.isNotEmpty
          ? null
          : (date != null && isOffList)
              ? ProductStoreHelpers.get_selected_bad_product_record(
...
class GroupedBadProductRecordModel {
  List<BadProductRecordModel> record;
  DateTime date;

  GroupedBadProductRecordModel({
