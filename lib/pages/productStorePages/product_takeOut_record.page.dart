...
class ProductTakeOutRecordPage extends StatefulWidget {
  const ProductTakeOutRecordPage({super.key});

  @override
  State<ProductTakeOutRecordPage> createState() =>
      _ProductTakeOutRecordPageState();
}

class _ProductTakeOutRecordPageState extends State<ProductTakeOutRecordPage> {
  List<ProductTakeOutRecordModel> auth_record = [];
  List<GroupedProductTakeOutRecordModel> grouped_record = [];
  List<ProductTakeOutRecordModel> selected_record = [];
  List<ProductTakeOutRecordModel> pending_record = [];
  ...
  get_record() {
    List<ProductTakeOutRecordModel> all_record =
        Provider.of<AppData>(context).product_takeOut_record;
    ...
    groups.forEach((key, value) {
      GroupedProductTakeOutRecordModel val =
          GroupedProductTakeOutRecordModel(date: key, record: value);
  ...
  // date picker tile
  Widget date_picker_tile(GroupedProductTakeOutRecordModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
  ...
  // record view
  Widget record_view(List<ProductTakeOutRecordModel> _record) {
    ...
    return Container(
      child: _record.isNotEmpty
          ? StickyGroupedListView(
              ...
              elements: _record,
              groupBy: (ProductTakeOutRecordModel element) =>
                  UniversalHelpers.get_date(element.recordDate!),
              groupComparator: (DateTime value1, DateTime value2) =>
                  value2.compareTo(value1),
              groupSeparatorBuilder: (ProductTakeOutRecordModel element) =>
                  group_separator(element.recordDate!),
              itemBuilder: (context, ProductTakeOutRecordModel element) =>
                  record_tile(element),
  ...
  // record tile
  Widget record_tile(ProductTakeOutRecordModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Text('Taken by : ', style: label_style),
                    SizedBox(width: 4),
                    Text(record.takenBy?.nickName ?? 'No Staff',
                        style: main_style),
                    Expanded(child: Container()),
                ...
                onPressed: () async {
                  ...
                  if (res != null) {
                    ...
                    if (res == 'Authorize') {
                      ...
                      ProductStoreHelpers.verify_product_takeOut_record(
                          context,
                    ...
                    if (res == 'edit') {
                      ...
                      if (width > 600) {
                        showDialog(
                            ...
                            builder: (context) => EnterProductTakeOutPage(
                                  editModel: record,
                                ));
                      } else {
                        Navigator.push(
                    ...
                    if (res == 'delete') {
                      ProductStoreHelpers.delete_product_takeOut_record(
  ...
  Widget view_record_by_date(
    ...
    List<ProductTakeOutRecordModel> records = [];
    ...
    return FutureBuilder<List<ProductTakeOutRecordModel>>(
      initialData: records,
      future: records.isNotEmpty
          ? null
          : (date != null && isOffList)
              ? ProductStoreHelpers.get_selected_product_takeOut_record(
...
class GroupedProductTakeOutRecordModel {
  List<ProductTakeOutRecordModel> record;
  DateTime date;

  GroupedProductTakeOutRecordModel({
