...
class ProductReceivedRecordPage extends StatefulWidget {
  const ProductReceivedRecordPage({super.key});

  @override
  State<ProductReceivedRecordPage> createState() =>
      _ProductReceivedRecordPageState();
}

class _ProductReceivedRecordPageState extends State<ProductReceivedRecordPage> {
  List<ProductReceivedRecordModel> auth_record = [];
  List<GroupedProductReceivedRecordModel> grouped_record = [];
  List<ProductReceivedRecordModel> selected_record = [];
  List<ProductReceivedRecordModel> pending_record = [];
  ...
  get_record() {
    List<ProductReceivedRecordModel> all_record =
        Provider.of<AppData>(context).product_received_record;
    ...
    groups.forEach((key, value) {
      GroupedProductReceivedRecordModel val =
          GroupedProductReceivedRecordModel(date: key, record: value);
  ...
  // date picker tile
  Widget date_picker_tile(GroupedProductReceivedRecordModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
  ...
  // record view
  Widget record_view(List<ProductReceivedRecordModel> _record) {
    ...
    return Container(
      child: _record.isNotEmpty
          ? StickyGroupedListView(
              ...
              elements: _record,
              groupBy: (ProductReceivedRecordModel element) =>
                  UniversalHelpers.get_date(element.recordDate!),
              groupComparator: (DateTime value1, DateTime value2) =>
                  value2.compareTo(value1),
              groupSeparatorBuilder: (ProductReceivedRecordModel element) =>
                  group_separator(element.recordDate!),
              itemBuilder: (context, ProductReceivedRecordModel element) =>
                  record_tile(element),
  ...
  // record tile
  Widget record_tile(ProductReceivedRecordModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    SizedBox(width: 4),
                    Text(record.receiver?.nickName ?? 'No Staff',
                        style: main_style),
                    Expanded(child: Container()),
                ...
                // supplier
                if (record.receiver != null && record.supplier!.isNotEmpty)
                  Row(
                    children: [
            ...
            child: IconButton(
                onPressed: () async {
                  ...
                  if (res != null) {
                    ...
                    if (res == 'delete') {
                      ProductStoreHelpers.delete_product_received_record(
                          context, record.key!);
                    }
                  }
                },
  ...
  Widget view_record_by_date(
    ...
    List<ProductReceivedRecordModel> records = [];
    ...
    return FutureBuilder<List<ProductReceivedRecordModel>>(
      initialData: records,
      future: records.isNotEmpty
          ? null
          : (date != null && isOffList)
              ? ProductStoreHelpers.get_selected_product_received_record(
              ...
              : (month != null && isOffList)
                  ...
                  : (dateRange != null && isOffList)
                      ? ProductStoreHelpers
                          .get_selected_product_received_record(context, {
                          'timeFrame': [
...
class GroupedProductReceivedRecordModel {
  List<ProductReceivedRecordModel> record;
  DateTime date;

  GroupedProductReceivedRecordModel({
