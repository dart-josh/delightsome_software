...
class RestockProductMaterialRecordPage extends StatefulWidget {
  const RestockProductMaterialRecordPage({super.key});

  @override
  State<RestockProductMaterialRecordPage> createState() =>
      _RestockProductMaterialRecordPageState();
}

class _RestockProductMaterialRecordPageState
    extends State<RestockProductMaterialRecordPage> {
  List<RestockProductMaterialModel> auth_record = [];
  List<GroupedRestockProductMaterialModel> grouped_record = [];
  List<RestockProductMaterialModel> selected_record = [];
  List<RestockProductMaterialModel> pending_record = [];
  ...
  get_record() {
    List<RestockProductMaterialModel> all_record =
        Provider.of<AppData>(context).restock_product_material_record;

    pending_record =
        all_record.where((element) => !element.verified!).toList();
    ...
    groups.forEach((key, value) {
      GroupedRestockProductMaterialModel val =
          GroupedRestockProductMaterialModel(date: key, record: value);
    ...
    return Container(
      ...
      child: Stack(
        children: [
          ...
          Center(
            child: Text(
              title ?? 'Restock Product Material Record',
              style: TextStyle(
                color: Colors.white,
                fontSize: 16,
  ...
  // date picker tile
  Widget date_picker_tile(GroupedRestockProductMaterialModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
  ...
  // record view
  Widget record_view(List<RestockProductMaterialModel> _record) {
    ...
    return Container(
      child: _record.isNotEmpty
          ? StickyGroupedListView(
              ...
              elements: _record,
              groupBy: (RestockProductMaterialModel element) =>
                  UniversalHelpers.get_date(element.recordDate!),
              groupComparator: (DateTime value1, DateTime value2) =>
                  value2.compareTo(value1),
              groupSeparatorBuilder: (RestockProductMaterialModel element) =>
                  group_separator(element.recordDate!),
              itemBuilder: (context, RestockProductMaterialModel element) =>
                  record_tile(element),
  ...
  // record tile
  Widget record_tile(RestockProductMaterialModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
                ...
                if (record.shortNote != null && record.shortNote!.isNotEmpty)
                  Row(
                    children: [
                      ...
                      SizedBox(width: 4),
                      Expanded(child: Text(record.shortNote!, style: note_style)),
                    ],
                  ),
  ...
  Widget view_record_by_date(
    ...
    List<RestockProductMaterialModel> records = [];
    ...
    return FutureBuilder<List<RestockProductMaterialModel>>(
      initialData: records,
      future: records.isNotEmpty
          ...
          : (date != null && isOffList)
              ...
              : (month != null && isOffList)
                  ? MaterialStoreHelpers
                      .get_selected_restock_product_materials_record(context, {
...
class GroupedRestockProductMaterialModel {
  List<RestockProductMaterialModel> record;
  DateTime date;

  GroupedRestockProductMaterialModel({
