...
class AddUpdateRawMaterial extends StatefulWidget {
  final RawMaterialsModel? item;
  const AddUpdateRawMaterial({super.key, this.item});

  @override
  State<AddUpdateRawMaterial> createState() => _AddUpdateRawMaterialState();
}

class _AddUpdateRawMaterialState extends State<AddUpdateRawMaterial> {
  ...
  List<String> measurementUnit_options = ['Weight', 'Count'];

  String storeType = '';

  List<String> storeType_options = [
    'Store',
    'Processed',
  ];
  ...
  void get_categories() {
    var cats = Provider.of<AppData>(context).raw_material_categories;
  ...
  void get_values() {
    if (widget.item == null) {
      ...
      measurementUnit = widget.item!.measurementUnit;
      storeType = widget.item!.storeType;
      edit = false;
      new_item = false;
    }
  }
        ...
        children: [
          ...
          Container(
            ...
            child: Select_form(
              ...
              options: measurementUnit_options,
              text_value: measurementUnit,
              edit: edit,
              setval: (val) {
                if (val != null) {
                  measurementUnit = val;
        ...
        onTap: () async {
          ...
          if (measurementUnit.isEmpty) {
            UniversalHelpers.showToast(
              context: context,
              color: Colors.redAccent,
              toastText: 'Select a unit of measurement',
              icon: Icons.error,
            );
            return;
          }
          ...
          if (response != null && response) {
            RawMaterialsModel new_item = RawMaterialsModel(
              key: widget.item?.key,
            ...
            bool done = await MaterialStoreHelpers.add_update_raw_materials(
                context, map);
            if (done) {
