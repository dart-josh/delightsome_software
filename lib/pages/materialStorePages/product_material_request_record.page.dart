...
class ProductMaterialRequestRecordPage extends StatefulWidget {
  const ProductMaterialRequestRecordPage({super.key});

  @override
  State<ProductMaterialRequestRecordPage> createState() =>
      _ProductMaterialRequestRecordPageState();
}

class _ProductMaterialRequestRecordPageState
    extends State<ProductMaterialRequestRecordPage> {
  List<ProductMaterialsRequestModel> auth_record = [];
  List<GroupedProductMaterialsRequestModel> grouped_record = [];
  List<ProductMaterialsRequestModel> selected_record = [];
  List<ProductMaterialsRequestModel> pending_record = [];
  ...
  get_record() {
    List<ProductMaterialsRequestModel> all_record =
        Provider.of<AppData>(context).product_material_request_record;
    ...
    groups.forEach((key, value) {
      GroupedProductMaterialsRequestModel val =
          GroupedProductMaterialsRequestModel(date: key, record: value);
    ...
    return Container(
      ...
      child: Stack(
        children: [
          ...
          Center(
            child: Text(
              title ?? 'Product Material Request Record',
              style: TextStyle(
                color: Colors.white,
                fontSize: 16,
  ...
  // date picker tile
  Widget date_picker_tile(GroupedProductMaterialsRequestModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
  ...
  // record view
  Widget record_view(List<ProductMaterialsRequestModel> _record) {
    ...
    return Container(
      child: _record.isNotEmpty
          ? StickyGroupedListView(
              ...
              elements: _record,
              groupBy: (ProductMaterialsRequestModel element) =>
                  UniversalHelpers.get_date(element.recordDate!),
              groupComparator: (DateTime value1, DateTime value2) =>
                  value2.compareTo(value1),
              groupSeparatorBuilder: (ProductMaterialsRequestModel element) =>
                  group_separator(element.recordDate!),
              itemBuilder: (context, ProductMaterialsRequestModel element) =>
                  record_tile(element),
  ...
  // record tile
  Widget record_tile(ProductMaterialsRequestModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
            ...
            child: Column(
              ...
              children: [
                ...
                if (record.purpose != null && record.purpose!.isNotEmpty)
                  Row(
                    children: [
                      Text('Purpose : ', style: label_style),
                      SizedBox(width: 4),
  ...
  Widget view_record_by_date(
    ...
    List<ProductMaterialsRequestModel> records = [];
    ...
    return FutureBuilder<List<ProductMaterialsRequestModel>>(
      initialData: records,
      future: records.isNotEmpty
          ...
          : (date != null && isOffList)
              ...
              : (month != null && isOffList)
                  ? MaterialStoreHelpers
                      .get_selected_product_materials_request_record(context, {
...
class GroupedProductMaterialsRequestModel {
  List<ProductMaterialsRequestModel> record;
  DateTime date;

  GroupedProductMaterialsRequestModel({
