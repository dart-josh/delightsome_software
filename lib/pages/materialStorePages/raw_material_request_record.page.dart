...
class RawMaterialRequestRecordPage extends StatefulWidget {
  const RawMaterialRequestRecordPage({super.key});

  @override
  State<RawMaterialRequestRecordPage> createState() =>
      _RawMaterialRequestRecordPageState();
}

class _RawMaterialRequestRecordPageState
    extends State<RawMaterialRequestRecordPage> {
  List<RawMaterialsRequestModel> auth_record = [];
  List<GroupedRawMaterialsRequestModel> grouped_record = [];
  List<RawMaterialsRequestModel> selected_record = [];
  List<RawMaterialsRequestModel> pending_record = [];
  ...
  get_record() {
    List<RawMaterialsRequestModel> all_record =
        Provider.of<AppData>(context).raw_material_request_record;
    ...
    groups.forEach((key, value) {
      GroupedRawMaterialsRequestModel val =
          GroupedRawMaterialsRequestModel(date: key, record: value);
    ...
    return Container(
      ...
      child: Stack(
        children: [
          ...
          Center(
            child: Text(
              title ?? 'Raw Material Request Record',
              style: TextStyle(
                color: Colors.white,
                fontSize: 16,
  ...
  // date picker tile
  Widget date_picker_tile(GroupedRawMaterialsRequestModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
  ...
  // record view
  Widget record_view(List<RawMaterialsRequestModel> _record) {
    ...
    return Container(
      child: _record.isNotEmpty
          ? StickyGroupedListView(
              ...
              elements: _record,
              groupBy: (RawMaterialsRequestModel element) =>
                  UniversalHelpers.get_date(element.recordDate!),
              groupComparator: (DateTime value1, DateTime value2) =>
                  value2.compareTo(value1),
              groupSeparatorBuilder: (RawMaterialsRequestModel element) =>
                  group_separator(element.recordDate!),
              itemBuilder: (context, RawMaterialsRequestModel element) =>
                  record_tile(element),
  ...
  // record tile
  Widget record_tile(RawMaterialsRequestModel record) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
  ...
  Widget view_record_by_date(
    ...
    List<RawMaterialsRequestModel> records = [];
    ...
    return FutureBuilder<List<RawMaterialsRequestModel>>(
      initialData: records,
...
class GroupedRawMaterialsRequestModel {
  List<RawMaterialsRequestModel> record;
  DateTime date;

  GroupedRawMaterialsRequestModel({
