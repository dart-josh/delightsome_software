...
class EnterProductMaterialReturn extends StatefulWidget {
  final ProductMaterialsReturnModel? editModel;
  const EnterProductMaterialReturn({super.key, this.editModel});

  @override
  State<EnterProductMaterialReturn> createState() =>
      _EnterProductMaterialReturnState();
}

class _EnterProductMaterialReturnState
    extends State<EnterProductMaterialReturn> {
  ...
  List<ProductMaterialsModel> product_materials = [];

  List<ProductMaterialItemModel> selected_items = [];

  List<ProductMaterialsModel> search_items = [];
  ...
  get_items() {
    product_materials = Provider.of<AppData>(context).product_material_list;
  }
              ...
              InkWell(
                onTap: () {
                  ...
                  if (selected_items.isNotEmpty) {
                    var pmrm = ProductMaterialsReturnModel(
                    ...
                    if (widget.editModel?.key == null) {
                      saved_product_material_return_model = pmrm;
                    }
                  } else {
                    saved_product_material_return_model = null;
                  }
      ...
      child: Row(
        children: [
          ...
          if (!search_open || (width > 600))
            InkWell(
              onTap: () async {
                ProductMaterialsModel? item = await showDialog(
                  context: context,
                  builder: (context) => ProductMaterialSelector(
  ...
  // search tile
  Widget search_tile(ProductMaterialsModel item) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
    double width = MediaQuery.of(context).size.width;
  ...
  // selected Items tile
  Widget selected_items_tile(ProductMaterialItemModel item) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
            ...
            child: InkWell(
              onTap: () async {
                ...
                if (res != null) {
                  ...
                  if (widget.editModel?.key == null) {
                    var pmrm_s = ProductMaterialsReturnModel(
                      items: selected_items,
                      purpose: purpose,
                      returnedBy: staff,
                    );
                    saved_product_material_return_model = pmrm_s;
                  }
                  ...
                  Map map = pmrm.enter_toJson(
                      returnedBy: staff!.key!, editedBy: auth_staff.key ?? '');

                  bool done = await MaterialStoreHelpers
                      .enter_product_materials_return_record(context, map);
...
// search items
  void search(String value) {
    if (value.isNotEmpty) {
      search_on = true;
      search_items = product_materials
          .where((item) =>
  ...
  Future<void> add_item(ProductMaterialsModel item) async {
    ...
    if (chk.isEmpty) {
      ...
      if (res != null) {
        selected_items.add(
          ProductMaterialItemModel(item: item, quantity: res),
        );
        search_on = false;
        setState(() {});
      }
