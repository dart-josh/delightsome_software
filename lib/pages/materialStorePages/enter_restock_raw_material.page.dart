...
class EnterRestockRawMaterial extends StatefulWidget {
  final RestockRawMaterialModel? editModel;
  const EnterRestockRawMaterial({super.key, this.editModel});

  @override
  State<EnterRestockRawMaterial> createState() =>
      _EnterRestockRawMaterialState();
}

class _EnterRestockRawMaterialState extends State<EnterRestockRawMaterial> {
  ...
  List<RawMaterialItemModel> selected_itemsUsed = [];
  ...
  void get_edit_values() {
    if (widget.editModel != null) {
      ...
      selected_itemsUsed = widget.editModel!.itemsUsed;
              ...
              InkWell(
                onTap: () {
                  ...
                  if (selected_items.isNotEmpty) {
                    var rrmm = RestockRawMaterialModel(
                      items: selected_items,
                      itemsUsed: selected_itemsUsed,
                      shortNote: shortNote,
                      receiver: staff,
                    );

                    if (widget.editModel?.key == null) {
                      saved_restock_raw_material_model = rrmm;
                    }
                  } else {
  ...
  // item list
  Widget item_list(List<RawMaterialItemModel> item_list) {
    bool isDarkTheme =
        Provider.of<AppData>(context).themeMode == ThemeMode.dark;
  ...
  Widget selected_items_tile(RawMaterialItemModel item) {
    ...
    int sn = index == 0
        ? selected_items.indexOf(item) + 1
        : selected_itemsUsed.indexOf(item) + 1;
          ...
          InkWell(
            onTap: () {
              if (index == 0)
                ...
                selected_itemsUsed.remove(item);
              setState(() {});
            },
            child: Container(
              width: 50,
            ...
            child: InkWell(
              onTap: () async {
                ...
                if (res != null) {
                  ...
                  if (widget.editModel?.key == null) {
                    var rrmm_s = RestockRawMaterialModel(
                      items: selected_items,
                      itemsUsed: selected_itemsUsed,
                      shortNote: shortNote,
                      receiver: staff,
...
// search items
  ...
  Future<void> add_item(RawMaterialsModel item) async {
    var chk = index == 0 ? selected_items.where((e) => e.item.key == item.key) : selected_itemsUsed.where((e) => e.item.key == item.key);
    ...
    if (chk.isEmpty) {
      ...
      if (res != null) {
        index == 0 ? selected_items.add(
          RawMaterialItemModel(item: item, quantity: res),
        ) : selected_itemsUsed.add(
          RawMaterialItemModel(item: item, quantity: res),
        );
